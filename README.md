...
| `MAIL_USERNAME` | vazio | conta Gmail usada pra enviar e-mail via SMTP — fallback, usado só se `RESEND_API_KEY` estiver vazia |
| `MAIL_PASSWORD` | vazio | senha de app do Gmail (não a senha normal da conta) dessa mesma conta |
| `RESEND_API_KEY` | vazio | API key do [Resend](https://resend.com) — **preferido em produção**, porque manda por HTTPS em vez de SMTP (hosts free tier, Render incluso, costumam bloquear porta SMTP de saída) |
| `RESEND_FROM` | `onboarding@resend.dev` | endereço remetente no Resend (opcional) |
