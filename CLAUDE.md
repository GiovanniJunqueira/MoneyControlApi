...
- **Mês = mês de calendário escolhido pelo usuário, não "hoje até eu clicar de novo"**: `POST /bets/months` recebe `year`/`month` (não usa mais `LocalDate.now()` como início) — `startMonth()` sempre usa o dia 1 daquele mês como `startDate`. Três casos, todos tratados em `startMonth()`:
  ...
  - **Consequência aceita**: como o `startingBanca` de um mês de backfill não é "de verdade" contínuo com o resto do histórico, ele **não entra na conta do "lucro total"** do topo (`overview()` usa o mês criado primeiro **de verdade**, por `createdAt`, como âncora — não o mais antigo por calendário) — o mês de backfill aparece com seu próprio lucro na lista, mas isolado do total.
...
- **`houseSummaries` (resumo por casa do mês inteiro)**: `computeMonthDays()` já varre dia a dia calculando o `result`/`resultUnits` de cada casa — o resumo é só um acumulador (`Map<UUID, BigDecimal>` com `.merge(id, result, BigDecimal::add)`) somado ao longo do mesmo loop, sem nenhuma query extra. **Importante**: essa soma pode não bater com `profitLoss` do mês (que é `endingBanca − startingBanca`, banca real) quando existe algum ajuste de saldo inicial (depósito/saque) no meio do mês — isso é esperado, não é bug: o `houseSummaries` soma só o *resultado de aposta* (que por definição exclui movimentações manuais), enquanto `profitLoss` reflete a banca real, incluindo o que entrou/saiu.
