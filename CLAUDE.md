...
- **Modelo** (`V4__add_bets.sql`, `V5__bet_unit_value_change_created_at.sql`): `bet_houses` (casas de aposta do usuário), `bet_months` (um "mês" de acompanhamento — só pode existir **um aberto** por usuário, garantido por índice único parcial `WHERE end_date IS NULL`), `bet_unit_value_changes` (log append-only do valor da unidade ao longo do tempo) e `bet_daily_balances` (uma entrada de saldo por casa por dia, unique `(house_id, date)`).
- **Unidade monetária com histórico**: a pessoa define um valor de unidade (ex: R$10 = 1un) ao iniciar o mês, e pode mudar em qualquer dia — o novo valor vale a partir daquele dia, sem alterar dias anteriores. Resolvido em `BetService.resolveUnitValue()` via `findTopByMonthIdAndDateLessThanEqualOrderByDateDescCreatedAtDesc`: **cuidado**, duas mudanças no mesmo dia empatam em `date`, por isso o desempate por `createdAt` (bug real encontrado e corrigido nesta sessão — sem o desempate, a query podia devolver a entrada errada de forma não-determinística).
